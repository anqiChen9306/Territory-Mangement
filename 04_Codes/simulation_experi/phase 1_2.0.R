...
random_100000_comb <- sample(1:nrow(salesmen_allocation_container_m), 100000, replace = F)
salesmen_allocation_container_m1 <- as.data.frame(salesmen_allocation_container_m[random_100000_comb,], 
                                                  stringsAsFactors = F)
...
registerDoSNOW(cl)
iterations <- nrow(salesmen_allocation_container_m1)
...
system.time(
  contain_phase1 <- foreach(i = 1:iterations, #.combine = rbind,
                            ...
                            .packages = c("dplyr", "tidyr", "DT", 
                              ...
                              salesmen_comb <- salesmen_allocation_container_m1[i,]
                              ...
                              for (hosp_num in 1:10) {
                                ...
                                for (prod_num in 1:4) {
                                  ...
                                  salesmen_chk2 <- switch(EXPR = hosp_salesmen,
                                                         a = "sr1_cont_time",
                                                         b = "sr2_cont_time",
                                                         c = "sr3_cont_time",
                                                         d = "sr4_cont_time",
                                  ...
                                  sr_cont_time[[salesmen_chk2]] <- sr_cont_time[[salesmen_chk2]]+ perm_prod_hours
...
close(pb)
stopCluster(cl)
contain_phase1m <- contain_phase1[sapply(contain_phase1, is.list)]
print(length(contain_phase1m))
not_contain_phase1m <- contain_phase1[!sapply(contain_phase1, is.list)]
## calculation

cl <- makeCluster(4, outfile="") # number of cores. Notice 'outfile'
registerDoSNOW(cl)
iterations <- length(contain_phase1m)
pb <- txtProgressBar(min = 1, max = iterations, style = 3)
progress <- function(n) setTxtProgressBar(pb, n)
opts <- list(progress = progress)


system.time(tmist_output_phase1 <- foreach(i = 1:iterations, #.combine = c, 
                               .options.snow = opts,
                               .packages = c("dplyr", "tidyr", "DT", 
                                             "data.table", "reshape2",
                                             "stringr")) %dopar% {
                                               
                                               tmp <- contain_phase1m[[i]]
